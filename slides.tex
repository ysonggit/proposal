...
\begin{frame}{Combined Communication-Authority Graph}
  \begin{definition}
    \small{Given a set of robots $R =\{r_1, ..., r_n\}$, the
      \textbf{communication graph} is a graph in which each vertex is
      a robot, and there exists an undirected edge between two
      vertices if corresponding robots are neighbors.}
  \end{definition}
  \begin{definition}
...
\begin{frame}{Neighborhood Property}
  ...
  \end{definition}
  \begin{definition}
    \small {The \textbf{pivot robot} is the robot with the highest
      authority in $\G\two$ for which the neighborhood properties
      differ from its neighborhood properties in $\G\one$.}
  \end{definition}
  \input{pivot.tex}  
\end{frame}

\begin{frame}{Progress of Proof}{Total Order over Combined Communication-Authority Graph}
...
\begin{frame}{Objectives}{2. Algorithm Correctness}
  ...
  \end{definition}
  \input{kconnect.tex} \textcolor{scred}{Intuition: given a set of
    robots, tell if our algorithm can work with given lattice graph.}\\
...
\begin{frame}{Objectives}{3. Algorithm Optimization}
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \includemovie[toolbar,poster]{6cm}{4cm}{videos/s7.mp4}
    \end{column}
  \end{columns}
\end{frame}
