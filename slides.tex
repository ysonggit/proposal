...
\begin{frame}{1.Construct Authority Tree}{Decide to be root or descendant}
  ...
  \begin{columns}[T] % align columns
    \begin{column}{.45\textwidth}
      \begin{itemize}
      \item \footnotesize{\textcolor{red}{root}: Robot who has the highest
          authority}
      \item \footnotesize{\textcolor{blue}{descendant}: Robot selects the
          neighbor who sends the highest authority and has matching with it as
          its parent}
      \item \footnotesize{\textcolor{orange}{orphan}: Robot who has no parent}
      \end{itemize}     
    \end{column}%
    \begin{column}{.5\textwidth}
       \input{authtree.tex}
