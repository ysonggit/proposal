...
\begin{frame}{Information Space}
  ...
  %\begin{figure}
    ...
    %   \foreach \i in {1,...,10}
    %   \fill [opacity=0.4] (360/10*\i:1+rnd*1) circle [radius=.025] coordinate (mark-\i);
    %   % Join them up
    %   \fill [opacity=0.4,blue]
    %   (mark-1) \foreach \i in {2,...,10}{ -- (mark-\i) } -- cycle;
    % \end{tikzpicture}
...
\begin{frame}{Information Space}
  ...
  \end{definition}
    ...
    \begin{column}{0.4\textwidth}
      ...
      % \footnotesize{intersecting the result of
      %   set transition function with the observation preimage}
...
\begin{frame}{Operations on Double-Rectangle Range Space}
  ...
  \begin{itemize}
  \item Action Update: $T_{drect}\left(A(\eta_k), u_k \right)$
    $$T_{drect}\left(A(\eta_k), u_k \right)=DRAP\left(X_{free} \cap [A(\eta_k) \oplus \{ u_k \} \oplus DRAP\left(\Theta(u_k)\right)]\right)$$
  \item Observation Update: $O_{drect}\left(A(\eta_k), y_k\right)$
    $$O_{drect}\left(A(\eta_k), y_k\right)=AABB\left(H(y_k) \cap R_1\right)\cup AABB\left(H(y_k) \cap R_2\right)$$
  \end{itemize}
