...
%      lattice pattern.}
%       \includemovie[toolbar,poster,autoplay]{4.5cm}{4.5cm}{videos/square10.mp4}
%     \end{column}%
%     \begin{column}{.45\textwidth}
%       \textcolor{scred}{OUTPUT:} \small{Move to this pattern using
%         only local information.}
%       \includemovie[toolbar,poster,autoplay]{4.5cm}{4.5cm}{videos/hexagon10.mp4}
%     \end{column}%
%   \end{columns} 
% \end{block}
% \end{frame}
...
\begin{frame}{Input: Lattice Graph}
    ...
    \end{bclogo}
    \small{Given a range, call a lattice graph \textbf{self-consistent} for
    this range if, for any two paths with the same starting node,
    the distance between two ending nodes is less than or equal to
    the range, there exist edges between two ending nodes of the paths.}
    \begin{columns}[T] % align columns
      \begin{column}{.45\textwidth}
        \begin{figure}
          \centering
...
\begin{frame}{1.Construct Authority Tree}{Decide to be root or descendant}
  \begin{block}{The robots use these authorities to establish a
    ...
    \item Forms an authority containing only its own ID,
      compares it with the authorities of remaining messages.
    \end{enumerate} 
  \begin{columns}[T] % align columns
    \begin{column}{.45\textwidth}
      \begin{itemize}
...
\begin{frame}{Opening}{}
 \begin{block}{}
   ...
   \begin{column}{.4\textwidth}
     ...
     \begin{itemize}
     \item locally, if the number of its neighbors is less than its
       out-degree, or
     \item choosing the nearest position from received messages.
     \end{itemize}
    \end{column}%
    \begin{column}{.5\textwidth}
      \begin{animateinline}[
        begin={%
          \begin{tikzpicture}%
           [post/.style={->,>=stealth', semithick, draw=blue!50},
            node/.style={circle,fill=red!20,draw,font=\sffamily\small}]%
            \useasboundingbox (-1.5,0) rectangle (5,5);
          },
          end={\end{tikzpicture}}
        ]{10}
        \draw[fill=red] (3,3) -- (2.75,3) -- (3.25,3.25) -- (3,2.75)  -- cycle;
...
\section{Results}
% \begin{frame}{Simulation of Old Algorithm}{Octagon-square formation with 100 robots}
%   \begin{center}
%     \includemovie[toolbar,poster,autoplay]{7cm}{7cm}{videos/octagonsq1000009v12sr55.mp4}
%   \end{center}
% \end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%
\begin{frame}{Simulation of Algorithm}{Octagon-square formation with 30 robots}
  \begin{center}
    \includemovie[toolbar,poster,autoplay]{7cm}{7cm}{videos/Octsq_formation_shell.mp4}
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%
\begin{frame}{Simulation of Algorithm}{Hexagon-square formation with 30 robots}
  \begin{center}
    \includemovie[toolbar,poster,autoplay]{7cm}{7cm}{videos/Hexsq_formation_shell.mp4}
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%
\begin{frame}{Simulation of Algorithm}{Triangle-hexagon-square formation with 30 robots}
  \begin{center}
    \includemovie[toolbar,poster,autoplay]{7cm}{7cm}{videos/Trihexsq_formation_shell.mp4}
  \end{center}
\end{frame}
...
\begin{block}{Summary}
  \begin{itemize}
  \item Robots can form different types of repeated lattice patterns.
  ...
  \end{itemize}
\end{block}
\begin{block}{Ongoing Work: finite lattice graph}
\end{block}
\begin{block}{Opening Questions}
  \begin{itemize}
  \item Given a set of robots' positions, can we find an algorithm to
    tell whether their positions satisfy a lattice pattern?
  \item Given a lattice graph, can we find an algorithm to check
    whether self-consistent.
